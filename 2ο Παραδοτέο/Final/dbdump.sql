...
USE `SpaceXMissionDB`$$
DROP TRIGGER IF EXISTS `SpaceXMissionDB`.`mission_BEFORE_INSERT` $$
USE `SpaceXMissionDB`$$
CREATE DEFINER = CURRENT_USER TRIGGER SpaceXMissionDB.`mission_BEFORE_INSERT` BEFORE INSERT ON mission FOR EACH ROW
BEGIN
	IF (new.launch_date > new.return_date) THEN 
        SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Warning: launch date after return date';
    END IF;
END$$


USE `SpaceXMissionDB`$$
DROP TRIGGER IF EXISTS `SpaceXMissionDB`.`mission_BEFORE_UPDATE` $$
USE `SpaceXMissionDB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `SpaceXMissionDB`.`mission_BEFORE_UPDATE` BEFORE UPDATE ON `mission` FOR EACH ROW
BEGIN
	IF (new.launch_date > new.return_date) THEN 
        SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Warning: launch date after return date';
    END IF;
...
USE `SpaceXMissionDB`$$
DROP TRIGGER IF EXISTS `SpaceXMissionDB`.`payload_BEFORE_INSERT` $$
USE `SpaceXMissionDB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `SpaceXMissionDB`.`payload_BEFORE_INSERT` BEFORE INSERT ON `payload` FOR EACH ROW
BEGIN
	IF EXISTS (SELECT launch_date FROM mission 
			   WHERE mission.mission_number = new.mission_number 
               AND (mission.launch_date > new.payload_release_date 
					OR mission.return_date < new.payload_release_date)) THEN 
...
USE `SpaceXMissionDB`$$
DROP TRIGGER IF EXISTS `SpaceXMissionDB`.`payload_BEFORE_UPDATE` $$
USE `SpaceXMissionDB`$$
CREATE DEFINER = CURRENT_USER TRIGGER `SpaceXMissionDB`.`payload_BEFORE_UPDATE` BEFORE UPDATE ON `payload` FOR EACH ROW
BEGIN
IF EXISTS (SELECT launch_date FROM mission 
			   WHERE mission.mission_number = new.mission_number 
               AND (mission.launch_date > new.payload_release_date 
...
INSERT INTO `SpaceXMissionDB`.`mission` (`mission_number`, `name`, `budget`, `client`, `purpose`, `spaceship_serial`, `launch_sp_code`, `launch_date`, `return_sp_code`, `return_date`) VALUES (98034, 'AAB12', 88000000, 'Nasa', 'Exploration', '346872G', 'C39A', '2021/3/21', 'SPX1', '2022/5/25');
...
START TRANSACTION;
USE `SpaceXMissionDB`;
INSERT INTO `SpaceXMissionDB`.`payload` (`serial_number`, `type`, `initial_mass`, `final_mass`, `mission_number`, `destination_code`, `payload_release_date`) VALUES ('ERT21P', 'Space Probe', 41000, 40000 , 12545, 'ERT21P', '2021/5/20');
INSERT INTO `SpaceXMissionDB`.`payload` (`serial_number`, `type`, `initial_mass`, `final_mass`, `mission_number`, `destination_code`, `payload_release_date`) VALUES ('GHD98K', 'Satellite', 39000 , 30000, 67487, 'GHD98K', '2022/6/12');
INSERT INTO `SpaceXMissionDB`.`payload` (`serial_number`, `type`, `initial_mass`, `final_mass`, `mission_number`, `destination_code`, `payload_release_date`) VALUES ('FGW25F', 'Spacecraft', 46000 , 40000 , 63457, 'FGW25F', '2021/1/1');
INSERT INTO `SpaceXMissionDB`.`payload` (`serial_number`, `type`, `initial_mass`, `final_mass`, `mission_number`, `destination_code`, `payload_release_date`) VALUES ('FEW45K', 'Satellite', 42000 , 38800 , 42367, 'FEW45K', '2026/3/5');
INSERT INTO `SpaceXMissionDB`.`payload` (`serial_number`, `type`, `initial_mass`, `final_mass`, `mission_number`, `destination_code`, `payload_release_date`) VALUES ('FEG45L', 'Space Probe', 40000 , 36000 , 98034, 'FEG45L', '2021/6/7');
INSERT INTO `SpaceXMissionDB`.`payload` (`serial_number`, `type`, `initial_mass`, `final_mass`, `mission_number`, `destination_code`, `payload_release_date`) VALUES ('GHJ78I', 'Satellite', 46000 , 36000 , 78249, 'GHJ78I', '2023/5/17');
INSERT INTO `SpaceXMissionDB`.`payload` (`serial_number`, `type`, `initial_mass`, `final_mass`, `mission_number`, `destination_code`, `payload_release_date`) VALUES ('HDE56K', 'Space Probe', 40000 , 38800 , 42678, 'HDE56K', '2022/8/18');
INSERT INTO `SpaceXMissionDB`.`payload` (`serial_number`, `type`, `initial_mass`, `final_mass`, `mission_number`, `destination_code`, `payload_release_date`) VALUES ('JAS124', 'Spacecraft', 100000 , 90000 , 85709, 'JAS124', '2021/7/4');
